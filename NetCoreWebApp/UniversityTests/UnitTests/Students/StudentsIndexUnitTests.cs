...
namespace UniversityTests.UnitTests.Students
{
    ...
    public class StudentsIndexUnitTests
    {
        ...
        [Test]
        public void IndexMethod_StudentsWithGradesAndWithoutGrades()
        {
         
            var dbLayer = new Mock<IStudentsRepository>();
            dbLayer.Setup(d => d.GetStudents()).Returns(new List<Student>()
            {
                new Student{IdStudent=1, FirstName="Ja", LastName="Ktos", Address="Warszawa"},
                new Student{IdStudent=2, FirstName="Ty", LastName="Cos", Address="Warszawa"},
            });
            dbLayer.Setup(d => d.GetGrades(1)).Returns(new List<Grade>()
            {
                new Grade{IdGrade=1, IdStudent=1, GradeValue=2, Subject="Wychowanie fizyczne", SubjectType="Group 2"}
            });
            ...
            Assert.IsTrue(vm.ElementAt(1).AverageGrade == 0);
            Assert.IsTrue(vm.ElementAt(1).EctsSum == 0);
        }
